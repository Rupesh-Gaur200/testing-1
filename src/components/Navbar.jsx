...
function Navbar(){
   ...
   window.addEventListener("scroll",handleScroll)
    ...
    return(
        <div     className={`fixed z-[999] w-full px-20 py-8 flex justify-between  text-white opacity-100 "}`}>
            <div className="logo">
            ...
            <div className="links flex gap-8 font-['Neue Montreal']">
             ...
             {["Sign in","Login","Services","Coures","Contact us",].map((item,index)=>(
                <a key={index} className={`text-md capitalize font-semibold ${index===4 && "ml-32"} ${active ? "text-black":"text-white"}`}>{item}</a>
             ))}
